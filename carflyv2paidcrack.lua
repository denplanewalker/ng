...
local API_URL = "https://archangel-station.onrender.com/log"

-- Флаг, чтобы отправить лог только один раз за инжект
local hasSentLog = false

local function getHWID()
    -- Приоритет 1: Официальный ClientId (самый стабильный в 2026)
    local success, clientId = pcall(function()
        return RbxAnalyticsService:GetClientId()
    end)
    
    if success and clientId and clientId ~= "" and clientId ~= "Unknown" then
        return clientId
    end

    -- Приоритет 2: Эксплойтные функции
    if gethwid then
        return gethwid()
    elseif gethwuid then
        return gethwuid()
    ...
    -- Запасной вариант
    local fallbackId = (clientId or "Unknown") 
    if identifyexecutor then
        fallbackId = fallbackId .. "-" .. identifyexecutor()
    end
    return fallbackId
end

local function sendLog(key)
    if hasSentLog then return end  -- Защита от повторной отправки
    hasSentLog = true
    ...
    -- Silent отправка (без print/warn)
    pcall(function()
...
local ThemeManager = loadstring(game:HttpGet("https://raw.githubusercontent.com/PyhTeT/pasholnaxui/refs/heads/main/ThemeManager.lua"))()
...
local Window = Library:CreateWindow({
    Title = "Kefysik fly| V2",
    Footer = "по поже будет ;)",
    NotifySide = "Right",
    ShowCustomCursor = true
})

local Tabs = {
    CF = Window:AddTab("Carfly", "car"),
    Visuals = Window:AddTab("Visuals", "eye"),
    ["UI Settings"] = Window:AddTab("UI SETTINGS", "settings")
}

local CarflySection = Tabs.CF:AddLeftGroupbox("Fly Settings")
local CarflySpeedSection = Tabs.CF:AddRightGroupbox("Movement Settings")
local VisualsSection = Tabs.Visuals:AddLeftGroupbox("Camera Settings")
local SkyboxSection = Tabs.Visuals:AddLeftGroupbox("SkyBox Settings")
local BadTripSection = Tabs.Visuals:AddLeftGroupbox("Bad Trip")
local HitSoundsSection = Tabs.Visuals:AddRightGroupbox("HitSounds")
local BulletTracerSection = Tabs.Visuals:AddLeftGroupbox("BulletTracer")
local ChamsG = Tabs.Visuals:AddRightGroupbox("Settings")
local ArmChamsSection = Tabs.Visuals:AddLeftGroupbox("Arm Chams")
local XraySection = Tabs.Visuals:AddRightGroupbox("X-Ray Settings")
...
--- ### НАСТРОЙКИ И ЛОГИКА BAD TRIP ### ---
local BadTripSettings = {
    MusicEnabled = false,
    Volume = 1,
    ActiveSound = nil,
    ActiveVisual = nil,        -- ColorCorrectionEffect
}

local function ToggleBadTrip(state)
    if state then
        -- === Музыка ===
        if not BadTripSettings.ActiveSound then
            local sound = Instance.new("Sound")
            sound.Name = "BadTripMusic"
            sound.SoundId = "rbxassetid://140179571050556"   -- можно поменять на любой trippy трек
            sound.Volume = BadTripSettings.Volume
            sound.Looped = true
            sound.Parent = game:GetService("SoundService")
            sound:Play()
            BadTripSettings.ActiveSound = sound
        end

        -- === Визуальный эффект (радужный тинт экрана) ===
        if not BadTripSettings.ActiveVisual then
            local colorCorrection = Instance.new("ColorCorrectionEffect")
            colorCorrection.Name = "BadTripEffect"
            colorCorrection.Parent = game:GetService("Lighting")
            BadTripSettings.ActiveVisual = colorCorrection
        end

    else
        -- Выключаем
        if BadTripSettings.ActiveSound then
            BadTripSettings.ActiveSound:Stop()
            BadTripSettings.ActiveSound:Destroy()
            BadTripSettings.ActiveSound = nil
        end

        if BadTripSettings.ActiveVisual then
            BadTripSettings.ActiveVisual:Destroy()
            BadTripSettings.ActiveVisual = nil
        end
...
-- === UI для BadTrip ===
BadTripSection:AddToggle("BadTrip", {
    Text = "BaDtRiP",
    Default = false,
    Callback = function(Value)
        BadTripSettings.MusicEnabled = Value
        ToggleBadTrip(Value)
    end
})
...
--- ### ЛОГИКА ТРЕЙСЕРОВ (УЛУЧШЕННАЯ ВЕРСИЯ) ###
local BulletFolder = game.Workspace:FindFirstChild("Const") and game.Workspace.Const:FindFirstChild("Ignore")

local TracerSettings = {
    Enabled = false,
    Rainbow = false,
    Color = Color3.fromRGB(0, 255, 255),
    Thickness = 0.25,          -- ширина луча
    Lifetime = 0.6,            -- сколько секунд видим
    Transparency = 0.3,
    FaceCamera = true,         -- очень важно для красивого вида
    Texture = "rbxassetid://7071772513", -- хороший текстурный овал (можно поменять)
    LightEmission = 0.8,
    CurveSize = 0.15,          -- лёгкий изгиб для "motion blur" эффекта
}

local ActiveTracers = {}  -- таблица для хранения активных Beam'ов

local function createTracer(startPos: Vector3, endPos: Vector3)
    if not TracerSettings.Enabled then return end
    if (startPos - endPos).Magnitude < 0.5 then return end

    -- Создаём временный Part-контейнер (чтобы не засорять Workspace напрямую)
    ...
    tracerPart.Parent = BulletFolder or Workspace
    ...
    beam.Color = ColorSequence.new(TracerSettings.Rainbow and Color3.fromHSV(tick() % 5 / 5, 1, 1) or TracerSettings.Color)
    beam.Width0 = TracerSettings.Thickness
    beam.Width1 = TracerSettings.Thickness * 0.6  -- слегка сужается к концу
    beam.Transparency = NumberSequence.new(TracerSettings.Transparency)
    beam.LightEmission = TracerSettings.LightEmission
    beam.LightInfluence = 0
    beam.Texture = TracerSettings.Texture
    beam.TextureLength = 1
    beam.TextureSpeed = 0
    beam.FaceCamera = TracerSettings.FaceCamera
    beam.CurveSize0 = TracerSettings.CurveSize
    beam.CurveSize1 = -TracerSettings.CurveSize  -- небольшой изгиб
    beam.Segments = 10
...
-- Обновление rainbow и cleanup
RunService.Heartbeat:Connect(function()
    if not TracerSettings.Enabled then 
        -- Очищаем старые трейсеры при выключении
    ...
    -- Cleanup старых (на всякий случай)
    for i = #ActiveTracers, 1, -1 do
        local t = ActiveTracers[i]
        if not t.Part or not t.Part.Parent or (now - t.Created > TracerSettings.Lifetime + 1) then
            if t.Part and t.Part.Parent then t.Part:Destroy() end
            table.remove(ActiveTracers, i)
        end
    end
    ...
    if TracerSettings.Rainbow then
        ...
        for _, t in ipairs(ActiveTracers) do
            if t.Beam then
                t.Beam.Color = ColorSequence.new(rainbowColor)
            end
        end
    end
end)

-- Основной цикл отслеживания пуль (оставляем почти как было, но вызываем новую функцию)
local lastPositions = {}
local AllowedProjectiles = {
    ["Bullet"] = true,
    ["Arrow"] = true,
    ["BlueBullet"] = true
}
...
RunService.Heartbeat:Connect(function()
    if not (TracerSettings.Enabled and BulletFolder) then 
        lastPositions = {}
        return 
    end
    ...
    for _, obj in ipairs(BulletFolder:GetChildren()) do
        if AllowedProjectiles[obj.Name] and obj:IsA("BasePart") then
            local bulletId = obj:GetDebugId()
            currentBullets[bulletId] = true

            local currentPos = obj.Position
            local lastPos = lastPositions[bulletId]

            if lastPos then
                createTracer(lastPos, currentPos)
            end

            lastPositions[bulletId] = currentPos
        end
    end

    -- Очистка неактуальных пуль
    for id in pairs(lastPositions) do
        if not currentBullets[id] then
...
local function v419(v414)
    for _, v416 in ipairs(Workspace:GetChildren()) do
            ...
            for _, v418 in ipairs(v416:GetDescendants()) do
                if v418:IsA("BasePart") and v413(v418.Material) then
                    if v414 then
                        ...
                        v418.Transparency = xrayTransparency;
...
local function scanForVehicles()
    ...
    for _, obj in pairs(Workspace:GetDescendants()) do
        if obj:IsA("Model") then
            local hasSeat = obj:FindFirstChildWhichIsA("VehicleSeat")
            local isLikelyVehicle = obj:FindFirstChild("Body") or obj:FindFirstChild("Main") or obj.Name:match("Car") or obj.Name:match("Vehicle")
            if hasSeat or isLikelyVehicle then
                local mainPart = obj.PrimaryPart or obj:FindFirstChild("Body") or obj:FindFirstChildWhichIsA("BasePart", true)
                if mainPart then
...
local function applyStabilization(mainPart)
    if not vehicleSettings.stabilize then return end
    local currentCF = mainPart.CFrame
    local lookVector = currentCF.LookVector
    mainPart.CFrame = CFrame.new(currentCF.Position, currentCF.Position + Vector3.new(lookVector.X, 0, lookVector.Z))
end

local function isCandidate(m)
    return m:IsA("Model") and (m:FindFirstChild("HumanoidRootPart") or m:FindFirstChild("Head"))
end

local function track(m)
    if not m or tracked[m] or m == game.Players.LocalPlayer.Character then return end
    tracked[m] = true
end

local ArmChams = ArmChams or {
    Settings = {
        enabled = false,
        rainbow = false,
        colorHand = Color3.fromRGB(255, 255, 255),
        material = Enum.Material.ForceField,
        transparency = 0.5
    },
    Parts = {}
...
ArmChams.Update = function()
    ArmChams.Parts = {}
    local ignoreFolder = Workspace:FindFirstChild("Const") 
        and Workspace.Const:FindFirstChild("Ignore")
        and Workspace.Const.Ignore:FindFirstChild("FPSArms")
    if ignoreFolder then
        for _, part in pairs(ignoreFolder:GetDescendants()) do
            if part:IsA("BasePart") then
                table.insert(ArmChams.Parts, part)
            end
        end
    end
end
...
ArmChams.Apply = function()
    if not ArmChams.Settings.enabled then return end
    
    local colorToUse = ArmChams.Settings.colorHand
    if ArmChams.Settings.rainbow then
        colorToUse = Color3.fromHSV(tick() % 5 / 5, 1, 1)
    end

    for _, p in ipairs(ArmChams.Parts) do
        if p and p.Parent then 
            p.Material = ArmChams.Settings.material
            p.Color = colorToUse
            p.Transparency = ArmChams.Settings.transparency
        end
    end
end
...
--- ### ТАБЛИЦА ЗВУКОВ ### ---
local SoundIds = {
    OtherHit = "rbxassetid://140134596265975",
    NoteHit = "rbxassetid://110825574801547",
    MetalHit = "rbxassetid://95942836778998",
    RetroHit = "rbxassetid://109303290150567",
    SphereClank = "rbxassetid://139184101428152",
    ButtonHit = "rbxassetid://135712815039142",
    Sahur = "rbxassetid://76635528617446",
...
local function UpdateHitSound(soundName)
    SelectedSound = soundName
    local soundId = SoundIds[soundName] or SoundIds.Default
    local sound = game:GetService("SoundService"):FindFirstChild("PlayerHitHeadshot")
    if sound then
        ...
        preview:Play()
        game:GetService("Debris"):AddItem(preview, 2)
    end
end
...
local function SetVolume(value)
    CurrentVolume = value
    local sound = game:GetService("SoundService"):FindFirstChild("PlayerHitHeadshot")
    if sound then sound.Volume = value end
end
...
local function ChangeSky(skyName)
    ...
    for _, obj in ipairs(l_Lighting_0:GetChildren()) do
        if obj:IsA("Sky") then obj:Destroy() end
    end
    if skyName == "Default" or not SkyAssets[skyName] then return end
    local l_Sky_0 = Instance.new("Sky")
    l_Sky_0.Name = "ExecutorSky"
    l_Sky_0.Parent = l_Lighting_0
    local assetId = SkyAssets[skyName]
    l_Sky_0.SkyboxBk = assetId
    l_Sky_0.SkyboxDn = assetId
    l_Sky_0.SkyboxFt = assetId
    l_Sky_0.SkyboxLf = assetId
    l_Sky_0.SkyboxRt = assetId
    l_Sky_0.SkyboxUp = assetId
end

--- ### ИНТЕРФЕЙС ### ---

CarflySection:AddToggle("EnableSystem", {
    Text = "Включить систему сканирования",
    Default = false,
...
local FlightToggle = CarflySection:AddToggle("FlightActive", {
    Text = "Активировать полёт",
    Default = false,
    Callback = function(state) vehicleSettings.flightActive = state end
})

FlightToggle:AddKeyPicker("FlightKeybind", {
    Default = "F", SyncToggleState = true, NoUI = false,
    Text = "Вкл/Выкл Полёт", Mode = "Toggle", 
    Callback = function(Value) vehicleSettings.flightActive = Value end,
})

CarflySection:AddToggle("NoClip", {
    Text = "NoClip-car",
    Default = false,
    Callback = function(state)
        ...
        if not state then
            for _, vehicle in pairs(vehicleSettings.activeVehicles) do
                if vehicle.mainPart then
                    vehicle.mainPart.CFrame = vehicle.mainPart.CFrame + Vector3.new(0, 7, 0)
...
CarflySection:AddToggle("Stabilize", {
    Text = "Auto Alignment",
    Default = true,
    Callback = function(state) vehicleSettings.stabilize = state end
})

local ArmToggle = ArmChamsSection:AddToggle("ArmChamsToggle", {
    Text = "Включить Chams (Руки)",
    Default = false,
    Callback = function(Value)
        ArmChams.Settings.enabled = Value
        if Value then
            ArmChams.Update()
            ArmChams.Apply()
        else
...
ArmToggle:AddColorPicker("ArmColor", {
    Default = ArmChams.Settings.colorHand,
    Title = "Цвет рук",
    Callback = function(Value) 
        ArmChams.Settings.colorHand = Value 
        if ArmChams.Settings.enabled then ArmChams.Apply() end
    end
})

ArmChamsSection:AddToggle("ArmRainbow", {
    Text = "Радужные руки (RGB)",
    Default = false,
    Callback = function(Value)
        ArmChams.Settings.rainbow = Value
        if ArmChams.Settings.enabled then ArmChams.Apply() end
    end
})

ArmChamsSection:AddDropdown("ArmMaterial", {
    Text = "Материал рук",
    Values = { "Plastic", "ForceField", "Neon", "Glass", "Wood", "Slate" },
    Default = "ForceField",
    Callback = function(Value) 
        ArmChams.Settings.material = Enum.Material[Value] 
        if ArmChams.Settings.enabled then ArmChams.Apply() end
    end
})

ArmChamsSection:AddSlider("ArmTransparency", {
    Text = "Прозрачность рук",
    Min = 0, Max = 1, Default = 0.5, Rounding = 1,
    Callback = function(Value) 
        ArmChams.Settings.transparency = Value 
        if ArmChams.Settings.enabled then ArmChams.Apply() end
    end
})

CarflySpeedSection:AddSlider("Speed", {
    Text = "Horizontal Speed", Min = 20, Max = 800, Default = 100, Rounding = 0,
    Callback = function(value) vehicleSettings.speed = value end
})

CarflySpeedSection:AddSlider("VSpeed", {
    Text = "Lifting Speed (N/Ctrl)", Min = 10, Max = 300, Default = 30, Rounding = 0,
    Callback = function(value) vehicleSettings.verticalSpeed = value end
})

CarflySpeedSection:AddSlider("TSpeed", {
    Text = "Turning Speed (←/→)", Min = 1, Max = 15, Default = 3, Rounding = 1,
    Callback = function(value) vehicleSettings.turnSpeed = value end
})

VisualsSection:AddToggle("EnableFOV", {
    Text = "FOV Changer",
    Default = false,
    Callback = function(state) cameraSettings.fovEnabled = state end
})

VisualsSection:AddSlider("FOVValue", { 
    Text = "FOV", Min = 30, Max = 120, Default = 70, Rounding = 0, 
    Callback = function(v) cameraSettings.fovValue = v end 
})

local PlayerChamsToggle = ChamsG:AddToggle("ChamsToggle", {
    Text = "Enable Chams (Players)",
    Default = Chams.enabled,
    Callback = function(v) Chams.enabled = v end
})

PlayerChamsToggle:AddColorPicker("ChamsColor", {
    Default = Chams.color,
    Callback = function(v) Chams.color = v end
})

ChamsG:AddToggle("PlayerRainbow", {
    Text = "Радужные игроки (RGB)",
    Default = false,
    Callback = function(Value)
        Chams.rainbow = Value
    end
...
ChamsG:AddDropdown("MatDrop", {
    Text = "Material",
    Values = { "Plastic", "ForceField", "Neon", "Glass" },
    Default = "ForceField",
    Callback = function(v) Chams.material = Enum.Material[v] end
})

ChamsG:AddSlider("Dist", {
    Text = "Distance", Default = 1000, Min = 1, Max = 5000, Rounding = 0,
    Callback = function(v) Chams.distance = v end
})

local TracerToggle = BulletTracerSection:AddToggle("TracerToggle", {
    Text = "Включить Трейсеры",
    Default = false,
    Callback = function(Value) TracerSettings.Enabled = Value end
})

TracerToggle:AddColorPicker("TracerColor", {
    Default = TracerSettings.Color,
    Title = "Цвет Трейсеров",
    Callback = function(Value) TracerSettings.Color = Value end
})

BulletTracerSection:AddToggle("TracerRainbow", {
    Text = "Радужные трейсеры (RGB)",
    Default = false,
    Callback = function(Value) TracerSettings.Rainbow = Value end
})

BulletTracerSection:AddSlider("TracerThickness", {
    Text = "Толщина",
    Min = 0.05, Max = 1, Default = 0.25, Rounding = 2,
    Callback = function(Value) TracerSettings.Thickness = Value end
})

BulletTracerSection:AddSlider("TracerLifetime", {
    Text = "Время жизни (сек)",
    Min = 0.1, Max = 2, Default = 0.6, Rounding = 2,
    Callback = function(Value) TracerSettings.Lifetime = Value end
})

BulletTracerSection:AddSlider("TracerTransparency", {
    Text = "Прозрачность",
    Min = 0, Max = 1, Default = 0.3, Rounding = 2,
    Callback = function(Value) TracerSettings.Transparency = Value end
})

BulletTracerSection:AddToggle("TracerFaceCamera", {
    Text = "Face Camera",
    Default = true,
    Callback = function(Value) TracerSettings.FaceCamera = Value end
})


BadTripSection:AddSlider("TripVolume", {
    Text = "Trip Volume",
    Min = 0, Max = 10, Default = 1, Rounding = 1,
    Callback = function(Value)
        if BadTripSettings then
            BadTripSettings.Volume = Value
            if BadTripSettings.ActiveSound then
                BadTripSettings.ActiveSound.Volume = Value
            end
        end
    end
})
...
XraySection:AddSlider("XrayTrans", {
    Text = "Прозрачность стен",
    Min = 0, Max = 1, Default = 0.5, Rounding = 1,
    Callback = function(Value)
        xrayTransparency = Value
        if v407 then v419(true) end
    end
})

SkyboxSection:AddDropdown("SkyboxDrop", {
    Text = "Смена Неба",
    Values = { "Defalt", "Magma", "Water", "Obsidian", "Galaxy", "Void" },
    Default = "Defalt",
    Callback = function(Value) ChangeSky(Value) end
})

HitSoundsSection:AddToggle("HitSoundToggle", {
    Text = "Звук попадания",
    Default = false,
    Callback = function(Value) if not Value then SetVolume(0) else SetVolume(CurrentVolume) end end
})

HitSoundsSection:AddSlider("HitSoundVol", {
    Text = "Громкость", Min = 0, Max = 10, Default = 1, Rounding = 1,
    Callback = function(Value) SetVolume(Value) end
})

HitSoundsSection:AddDropdown("HitSoundSelect", {
    Text = "Выбор звука",
    Values = { 
        "OtherHit", "NoteHit", "MetalHit", "RetroHit", 
        "SphereClank", "ButtonHit", "Sahur", 
        "Neverlose", "Rust","Default", "UwU" 
    },
    Default = "Default",
    Callback = function(Value) UpdateHitSound(Value) end
})

--- ### ЦИКЛЫ ОБНОВЛЕНИЯ ### ---

for _, m in next, Workspace:GetChildren() do if isCandidate(m) then track(m) end end
Workspace.ChildAdded:Connect(function(m) task.wait(0.1) if isCandidate(m) then track(m) end end)

RunService.RenderStepped:Connect(function()
    if cameraSettings.fovEnabled then
        Workspace.CurrentCamera.FieldOfView = cameraSettings.fovValue
    end
    
    if ArmChams.Settings.enabled and ArmChams.Settings.rainbow then
        ArmChams.Apply()
    end

       -- RGB Эффект экрана для Bad Trip
    if BadTripSettings.ActiveVisual then
        BadTripSettings.ActiveVisual.TintColor = Color3.fromHSV(tick() % 5 / 5, 0.7, 1)
    end
    
    local camPos = Workspace.CurrentCamera.CFrame.Position
    for m in next, tracked do
        if not m or not m.Parent then tracked[m] = nil continue end
        local hrp = m:FindFirstChild("HumanoidRootPart") or m:FindFirstChild("Head")
        if hrp then
            local dist = (hrp.Position - camPos).Magnitude
            local shouldBeOn = Chams.enabled and dist <= Chams.distance
            
            if shouldBeOn and Chams.rainbow then
                applyToModel(m, true)
            end
        end
    end
...
RunService.Heartbeat:Connect(function()
    if TracerSettings.Enabled and BulletFolder then 
        local currentBullets = {}
        for _, obj in ipairs(BulletFolder:GetChildren()) do
            if AllowedProjectiles[obj.Name] and obj:IsA("BasePart") then
                local bulletId = obj:GetDebugId()
                ...
                local lastPos = lastPositions[bulletId]
                if lastPos then
                    createBeam(lastPos, currentPos)
                end
                lastPositions[bulletId] = currentPos
            end
        end
        for id in pairs(lastPositions) do
            if not currentBullets[id] then lastPositions[id] = nil end
        end
    else
        lastPositions = {}
    end
    ...
    for m in next, tracked do
        ...
        if hrp then
            local dist = (hrp.Position - camPos).Magnitude
            if not Chams.rainbow or not (Chams.enabled and dist <= Chams.distance) then
                applyToModel(m, Chams.enabled and dist <= Chams.distance)
            end
        end
    end
    ...
    for _, vehicle in pairs(vehicleSettings.activeVehicles) do
        ...
        local hasDriver = vehicle.seat and vehicle.seat.Occupant ~= nil
        ...
        if vehicleSettings.flightActive and (hasDriver or not vehicle.seat) then
            ...
            if UserInputService:IsKeyDown(Enum.KeyCode.W) then velocity += moveDir * vehicleSettings.speed end
            if UserInputService:IsKeyDown(Enum.KeyCode.S) then velocity -= moveDir * vehicleSettings.speed end
            if UserInputService:IsKeyDown(Enum.KeyCode.A) then velocity -= Vector3.new(-moveDir.Z, 0, moveDir.X) * vehicleSettings.speed * 0.7 end
            if UserInputService:IsKeyDown(Enum.KeyCode.D) then velocity += Vector3.new(-moveDir.Z, 0, moveDir.X) * vehicleSettings.speed * 0.7 end
            
            if UserInputService:IsKeyDown(Enum.KeyCode.N) then velocity += Vector3.new(0, vehicleSettings.verticalSpeed, 0) end
            if UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) then velocity -= Vector3.new(0, vehicleSettings.verticalSpeed, 0) end

            if UserInputService:IsKeyDown(Enum.KeyCode.Left) then
                mainPart.CFrame = mainPart.CFrame * CFrame.Angles(0, math.rad(vehicleSettings.turnSpeed), 0)
            end
            if UserInputService:IsKeyDown(Enum.KeyCode.Right) then
                 mainPart.CFrame = mainPart.CFrame * CFrame.Angles(0, math.rad(-vehicleSettings.turnSpeed), 0)
            end

            applyStabilization(mainPart)
            mainPart.AssemblyLinearVelocity = velocity
            mainPart.AssemblyAngularVelocity = Vector3.zero
        end

        if vehicleSettings.noClip and vehicle.model then
            for _, part in pairs(vehicle.model:GetDescendants()) do
...
task.spawn(function()
    while true do
        if ArmChams.Settings.enabled then
            ArmChams.Update()
            if not ArmChams.Settings.rainbow then
                ArmChams.Apply()
            end
        end
        task.wait(1)
...
ThemeManager:SetFolder("KerfysikV2")
SaveManager:SetFolder("KerfysikV2/configs")
SaveManager:BuildConfigSection(Tabs["UI Settings"])
ThemeManager:ApplyToTab(Tabs["UI Settings"])
Library:SetWatermark("Kefysik FLY")
-- [NEW SCRIPT] --
